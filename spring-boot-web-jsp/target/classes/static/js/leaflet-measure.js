...
//  format area method
  ...
  onAdd: function (map) {
    ...
    this._createButton('&#8674;', '거리재기',
    'leaflet-control-measure leaflet-bar-part leaflet-bar-part-top-and-bottom',
    container, this._toggleMeasure, this)
    
    this._createButton('&#11091;', '면적재기',
    'leaflet-control-measure leaflet-bar-part leaflet-bar-part-top-and-bottom',
    container, this._toggleMeasureArea, this)
    
    this._createButton('&#128465;', '초기화',
    'leaflet-control-measure leaflet-bar-part leaflet-bar-part-top-and-bottom',
    container, this._clearMeasure, this)
  ...
  _toggleMeasure: function () {
    if( this.options.type == "Polyline" ) this._measuring = !this._measuring
  ...
  _toggleMeasureArea: function () {
	  if( this.options.type == "Polygon" ) this._measuring = !this._measuring
  ...
  _mouseMove: function (e) {
    ...
    if (!this._layerPaintPathTemp) {
      ...
      if(this.options.type == "Polyline"){
    	  ...
    	  this._layerPaintPathTemp = L.polygon([this._lastPoint, e.latlng], {
  	        color: this.options.lineColor,
  	        weight: this.options.lineWeight,
  	        opacity: this.options.lineOpacity,
  	        clickable: false,
      ...
      if( this.options.type == "Polyline" || !this._layerPaintPath || ( this._layerPaintPath && this._layerPaintPath.getLatLngs()[0].length < 2 ) ) this._layerPaintPathTemp.getLatLngs().splice(0, 2, this._lastPoint, e.latlng)
      else{
    	  var polygonPath = this._layerPaintPath.getLatLngs();
    	  this._layerPaintPathTemp.setLatLngs( polygonPath );
    ...
    if (this._tooltip) {
      if (this.options.type == "Polyline") {
    	  ...
    	  if( this._layerPaintPathTemp.getLatLngs()[0].length < 1 ) return;
    	  
    	  this._updateTooltipPosition(this._layerPaintPathTemp.getBounds().getCenter())
	      var area = L.GeometryUtil.geodesicArea(this._layerPaintPathTemp.getLatLngs()[0])
	      console.log(this._layerPaintPathTemp.getLatLngs());
  ...
  _mouseClick: function (e) {
    ...
    if( this.options.type == "Polyline" ){
      ...
      if( this._layerPaintPathTemp ){
    	  if (!this._tooltip) {
    		  this._createTooltip(this._layerPaintPathTemp.getBounds().getCenter(), [0, 0])
          }
    	  
    	  if( this._layerPaintPathTemp.getLatLngs()[0].length > 1 ){
  ...
  _finishPath: function (e) {
    ...
    if (this.options.type == "Polyline" && this._tooltip) {
  ...
  _createTooltip: function (position, iconAnchor) {
	if( !iconAnchor ) iconAnchor = [-5, -5];
  ...
  _updateTooltipArea: function (total) {
    ...
    var totalRound = this._formatArea(total)
  ...
  _formatArea: function (val) {
    if (typeof this.options.formatArea === 'function') {
      return this.options.formatArea(val);
    }
    if (val / 1000 < 1000) {
      return Math.round(val) / 1000 + 'm<sup>2</sup>'
    } else {
      return Math.round((val / 1000000 ) * 100) / 100 + 'km<sup>2</sup>'
    }
  },
